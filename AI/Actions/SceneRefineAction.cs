...
{
    public sealed class SceneRefineAction : IAiAction
    {
        public const string ActionIdValue = "scene.refine";

        public string ActionId => ActionIdValue;

        public string DisplayName => "Refine scene card";
        ...
        public AiRequest BuildRequest(AiActionInput input)
        {
            ...
            string instruction = string.IsNullOrWhiteSpace(input.Instruction)
                ? "Refine the existing scene card fields without changing the intent."
