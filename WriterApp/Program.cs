...
app.Use(async (context, next) =>
{
    ...
    try
    {
        if (enableAuthMeDiagnostics)
        {
            ...
            logger.LogInformation(
                "AuthMe diagnostics begin. Method={Method} Path={Path} Query={Query} XMsClientPrincipalLength={XMsClientPrincipalLength} XMsAadIdTokenLength={XMsAadIdTokenLength} AuthorizationScheme={AuthorizationScheme} AuthorizationLength={AuthorizationLength} CookieLength={CookieLength}",
    ...
    CancellationToken ct) =>
{
    ...
    bool force = ParseTruthy(context.Request.Query["force"]);
    if (force)
    {
